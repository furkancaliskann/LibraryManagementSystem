    ...
    <div className="max-w-6xl mx-auto mt-10 p-6 bg-white shadow rounded">
      ...
      {books.length === 0 ? (
        ...
        <table className="w-full table-auto border-collapse">
          ...
          <tbody>
            {books.map((book) => (
              <tr key={book.id} onClick={() => navigate(`/books/${book.id}`)} className="cursor-pointer hover:bg-gray-100">
                <td className="px-4 py-2 border text-center">{book.title}</td>
                <td className="px-4 py-2 border text-center">{book.author?.name || "-"}</td>
                <td className="px-4 py-2 border text-center">{book.category?.name || "-"}</td>
                <td className="px-4 py-2 border text-center">{book.publisher?.name || "-"}</td>
          ...
          <p className="text-gray-600 mb-6">{books.length} kitap listeleniyor.</p>
        </table>
      )}
    </div>
  );
